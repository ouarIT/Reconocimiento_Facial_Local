...
def getPromedios_local(pathPaths, path_Persona1, path_Persona2, path_Persona3):
    resultados_temp = []
    promedios = []
    ...
    for path in pathPaths:
        ...
        if path == path_Persona1:
            # guarda el promedio y el nombre de la persona
            # numero sumas de relaciones entre las fotos ingresadas
            promedios.append(
                [sum(resultados_temp) / len(resultados_temp), "Persona1"])
        elif path == path_Persona2:
            promedios.append(
                [sum(resultados_temp) / len(resultados_temp), "Persona2"])
        elif path == path_Persona3:
            promedios.append(
                [sum(resultados_temp) / len(resultados_temp), "Persona3"])
